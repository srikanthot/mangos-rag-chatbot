...
# Maximum total characters for the injected history block.
# Prevents context window overflow when many messages are loaded.
# ~12k chars ~ 3k tokens — safe headroom for system prompt + RAG context.
_MAX_HISTORY_BLOCK_CHARS = 12_000
...
def format_history_block(messages: list) -> str:
    ...
    Enforces a total character cap to prevent context window overflow.
    """
    if not messages:
        return ""
    ...
    for msg in messages:
        ...
        if total_chars + len(line) > _MAX_HISTORY_BLOCK_CHARS:
            logger.warning(
                "History block truncated at %d chars (cap=%d) — %d of %d messages included",
                total_chars, _MAX_HISTORY_BLOCK_CHARS, len(lines) - 1, len(messages),
            )
            break
        lines.append(line)
