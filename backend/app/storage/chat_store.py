...
_MAX_CAS_RETRIES = 5
_TRANSIENT_STATUS_CODES = {408, 429, 449, 503}
_MAX_TRANSIENT_RETRIES = 3
_TRANSIENT_BACKOFF_BASE = 0.5  # seconds


def _is_transient(exc: Exception) -> bool:
    """Return True if the Cosmos error is transient and worth retrying."""
    if isinstance(exc, CosmosHttpResponseError):
        return exc.status_code in _TRANSIENT_STATUS_CODES
    return False


async def _retry_transient(coro_factory, operation: str):
    """Retry an async Cosmos operation on transient failures with exponential backoff."""
    ...
    for attempt in range(_MAX_TRANSIENT_RETRIES):
        try:
            ...
            last_exc = exc
            if not _is_transient(exc) or attempt == _MAX_TRANSIENT_RETRIES - 1:
                raise
            wait = _TRANSIENT_BACKOFF_BASE * (2 ** attempt)
            logger.warning(
                "chat_store: transient error on %s (attempt %d/%d, retry in %.1fs) | %s",
                operation, attempt + 1, _MAX_TRANSIENT_RETRIES, wait, exc,
            )
...
def generate_title(question: str) -> str:
    ...
    if len(q) > 70:
        truncated = q[:67].rsplit(" ", 1)
        # If no space found (single long word), just hard-cut at 67 chars
        q = (truncated[0] if len(truncated) > 1 else q[:67]) + "…"
...
async def create_conversation(
    ...
    try:
        await _retry_transient(
            lambda: container.upsert_item(body=conv.model_dump(mode="json")),
            f"create_conversation(thread={thread_id})",
        )
        logger.info(
...
async def list_conversations(
    ...
    # Use single-field ORDER BY to avoid requiring a composite index in Cosmos.
    # Conversations with no messages yet (last_message_at = null) are sorted by
    # updated_at so they still appear at the top of the sidebar.
    query = (
        f"SELECT * FROM c WHERE c.user_id = @user_id {deleted_clause} "
        f"ORDER BY c.updated_at DESC OFFSET @offset LIMIT @limit"
    )
    params = [
...
async def _append_message(
    ...
    for attempt in range(_MAX_CAS_RETRIES):
        ...
        try:
            await _retry_transient(
                lambda: msg_container.upsert_item(body=msg.model_dump(mode="json")),
                f"upsert_message(thread={thread_id}, seq={sequence})",
            )
            logger.info(
