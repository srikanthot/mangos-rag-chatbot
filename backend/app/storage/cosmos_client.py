...
async def init_cosmos() -> None:
    ...
    try:
        ...
        if COSMOS_AUTO_CREATE_CONTAINERS:
            ...
            # Composite index enables multi-field ORDER BY queries.
            # Without it, Cosmos rejects e.g. ORDER BY last_message_at, created_at.
            # Note: create_container_if_not_exists won't update the index on an
            # existing container — use the Azure Portal or CLI to add it manually.
            conversations_indexing = {
            ...
            _conversations_container = await db.create_container_if_not_exists(
                ...
                default_ttl=ttl,
                indexing_policy=conversations_indexing,
            )
